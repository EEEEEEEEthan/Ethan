...
while (true)
{
    ...
    {
        if (!TryHandleSlash(trimmed, ref apiKey, ref model, ref baseUrl, messages, httpClient))
            break;
        continue;
    }
    ...
    try
    {
        var reply = await CompleteChatAsync(httpClient, baseUrl, apiKey, model, messages);
...
{
    private static readonly (string Replacement, string Caption)[] SlashCommands =
    [
        ("/apikey ", "设置 API Key"),
        ("/model ", "设置模型 id"),
        ("/url ", "OpenAI 兼容根地址"),
...
file sealed class SlashCompletionItem : CompletionItem
{
    ...
    public SlashCompletionItem(string replacement, string caption)
        : base(
            ...
            displayText: new FormattedString($"{replacement.TrimEnd()} — {caption}"),
