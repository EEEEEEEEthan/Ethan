...
namespace Agent;
static class DirectoryTreeTool
{
	sealed class FilteredTreeNode
	{
			...
			bool matchAll)
		{
			...
			catch { return selfMatches? node : null; }
	...
	internal const string jsonSchemaParameters = """
		{
		  "type": "object",
		  "properties": {
	...
	internal static string Invoke(string root, int maxDepth, string? filterPattern)
	{
		...
		catch(Exception exception) { return$"错误：无法解析路径：{exception.Message}"; }
		...
		try
		{
			var rootNode = FilteredTreeNode.Build(
				fullRoot,
				0,
				maxDepth,
				pattern,
		...
		catch(UnauthorizedAccessException) { return$"错误：无权限访问：{fullRoot}"; }
		catch(IOException exception) { return$"错误：{exception.Message}"; }
	}
	static void AppendRootLine(
		StringBuilder builder,
		string fullRoot,
		ref int linesUsed,
		int maxLines,
...
sealed record GetDirectoryTreeArguments(
	[property: JsonPropertyName("root")]string Root,
	[property: JsonPropertyName("max_depth")]
	int MaxDepth,
